...
from sqlalchemy import select, orm, insert, delete
...
async def remove_from_cart(product_id: int, user: User = Depends(current_user),
                           session: AsyncSession = Depends(get_async_session)):
    delete_stmt = delete(Cart).where(and_(Cart.user_id == user.id, Cart.product_id == product_id))
